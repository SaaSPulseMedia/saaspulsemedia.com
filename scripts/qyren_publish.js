 ...
 * Runs on GitHub's servers, no browser needed.
 * Writes a SaaS article and commits it directly to the repo.
 */
...
const SEED_TOPICS = [
  ...
  { topic: 'How to Build a Content Strategy Using Free SEO Tools', category: 'SEO Strategies' },
  ...
  { topic: 'Google Search Console vs Ahrefs: Which SEO Tool Do You Actually Need?', category: 'SEO Tools' },
  { topic: 'Zapier vs Make in 2026: Automation Platform Comparison', category: 'AI Tools for Business' },
];

// ── HELPERS ──────────────────────────────────────────────────
function log(msg) {
  console.log(`[QYREN] ${msg}`);
}
...
fetch('/data/posts.json?t='+Date.now()).then(r=>r.json()).then(all=>{
  ...
  const slug='${slug}';
  const related=all.filter(a=>a.slug!==slug&&a.category===cat).slice(0,3);
  if(!related.length)return;
  document.getElementById('related-section').style.display='block';
  document.getElementById('related-grid').innerHTML=related.map(r=>{
    const d=r.date?new Date(r.date).toLocaleDateString('en-GB',{day:'numeric',month:'short',year:'numeric'}):'';
    const link=r.url||('/articles/'+r.slug+'.html');
    return '<article class="article-card"><div class="card-image">'+(r.image?'<img src="'+r.image+'" alt="" loading="lazy" style="width:100%;height:100%;object-fit:cover">':'<div class="card-image-placeholder">📊</div>')+'</div><div class="card-body"><div class="card-cat">'+r.category+'</div><a href="'+link+'" class="card-title">'+r.title+'</a><p class="card-excerpt">'+(r.description||'').substring(0,130)+'</p><div class="card-meta"><span>'+d+'</span><a href="'+link+'" class="card-read">Read →</a></div></div></article>';
  }).join('');
}).catch(()=>{});
...
async function main() {
  ...
  // Load existing posts to avoid repeating topics
  let existingPosts = [];
  ...
  if (MANUAL_TOPIC) {
    ...
    // Pick from seed list, avoid recent topics
    ...
    decision = await callClaudeJSON(
      `You are QYREN, content strategist for SaaS Pulse Media. Pick the best SaaS article topic for today. The current year is 2026. Return JSON only.`,
      `Select a SaaS article topic. Recently covered: ${recentTopics || 'none'}.

Suggested starting point: "${seedPick.topic}" (category: ${seedPick.category})

You can use this suggestion or choose something more timely. Return JSON:
{
  ...
  "angle": "The unique editorial angle",
  "target_keywords": ["primary keyword", "secondary keyword"],
  "search_intent": "informational|commercial|navigational"
}`,
      600
    );
    log(`Topic selected: "${decision.topic}" [${decision.category}]`);
  }

  // ── STEP 2: Write metadata ──────────────────────────────────
  log('Generating article metadata...');
  const meta = await callClaudeJSON(
    `You are QYREN writing metadata for SaaS Pulse Media. The current year is 2026. Return JSON only.`,
...
Return JSON:
{
  "title": "SEO-optimised title with primary keyword (2026 where relevant)",
  "subtitle": "Engaging 1-sentence subtitle",
  "meta_description": "150-160 char SEO meta description",
  "seo_title": "Title tag version (55 chars max)",
  ...
  // ── STEP 3: Write article body ──────────────────────────────
  log('Writing article body...');
  const body = await callClaude(
    `You are QYREN writing for SaaS Pulse Media — a professional SaaS review publication. Write expert, practical, SEO-optimised content in ${STYLE} style. The current year is 2026 — always reference 2026, never 2025 as the current year. Return ONLY the article body text, no JSON, no preamble.`,
    `Write a complete ${ARTICLE_LENGTH}-word article:

Title: ${meta.title}
Category: ${decision.category}
Angle: ${decision.angle}
Target keywords (use naturally): ${(decision.target_keywords || []).join(', ')}

Requirements:
- Use ## for H2 headings, ### for H3
- Include numbered or bulleted lists where helpful
- Name specific real tools with realistic pricing
- Be specific — include real numbers, comparisons, examples
- End with a clear verdict or recommendation
- Do NOT include a title at the top — start with the first paragraph`,
    2800
  );
  log(`Article written: ${body.split(/\s+/).length} words`);
  ...
  // ── STEP 6: Update posts.json index ────────────────────────
  const indexEntry = {
    slug: article.slug,
    title: article.title,
  ...
  log(`posts.json updated: ${existingPosts.length} articles total`);

  // ── STEP 7: Generate social posts file ─────────────────────
  log('Generating social posts...');
  const social = await callClaudeJSON(
    `You are a social media strategist for SaaS Pulse Media. Write engaging platform-native posts. Return JSON only.`,
...
Return JSON:
{
  "twitter_thread": ["tweet 1 hook (max 280 chars)", "tweet 2 key point", "tweet 3 CTA with URL"],
  "twitter_single": "Single tweet with URL (max 280 chars)",
  "linkedin_post": "LinkedIn post 150-200 words. Professional. End with URL."
}`,
    800
  );

  // Save social posts to a file for reference
